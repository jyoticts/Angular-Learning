...
getPharmacyList (): Observable<any> {
  return this.get(_.extend(appApiResources['getPharmacyList']), {})
  .pipe(
  );
}
getGPOList (): Observable<any> {
  return this.get(_.extend(appApiResources['getGPOList']), {})
  .pipe(
  );
}
getGPODetails (gpoId): Observable<any> {
  return this.get(_.extend(appApiResources['getGPODetails'],{params: [gpoId] }), {})
  .pipe(
  );
}
getCalculations (qty,gpoId): Observable<any> {
  return this.get(_.extend(appApiResources['getCalculations'],{params: [qty,gpoId] }), {})
  .pipe(
  );
}
purchae (purchaseModel): Observable<any> {
  return this.post(_.extend(appApiResources['purchase']),purchaseModel, {responseType:"text"})
  .pipe(
  );
}
